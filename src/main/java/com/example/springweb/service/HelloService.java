...
public class HelloService {
    ...
    *自己写的部分：检查用户名和密码是否正确
    */
    public HelloUser loginverify(String username,String password){
        //System.out.println("loginverify entry");
        HelloUser result = helloMapper.verify(username,password);
        //System.out.println("loginverify:"+result);
        if (result==null)
        {
            result=new HelloUser();//索引为空的时候，返回null，需要这时候对其getId,getName就会出错。
        }
